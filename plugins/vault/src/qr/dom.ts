...
*/
/**
 * Optional DOM related utilities. Some utilities, useful to decode QR from
 * camera:
 * - draw overlay: helps user to position QR code on camera - draw bitmap:
 * useful for debugging (what decoder sees)
 * - draw result: show scanned QR code The code is fragile: it is easy to make
 * subtle errors, which will break decoding.
 *
 * @module
 */
...
export class QRCanvas {
  ...
  private drawResultQr({ data, height, width }: Image) {
    ...
    ;(this.resultQR.canvas as any).style =
...
class QRCamera {
  ...
  /**
   * Returns list of cameras NOTE: available only after first getUserMedia
   * request, so cannot be additional method.
   */
 ...
 *
 */
export async function frontalCamera(
