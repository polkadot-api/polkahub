...
function toBitmap(img: Image): Bitmap {
  ...
  for (let y = 0; y < bHeight; y++) {
    ...
    for (let x = 0; x < bWidth; x++) {
      ...
      for (
        let y = 0, pos = yPos * img.width + xPos;
        y < block;
        y += 1, pos += img.width
      ) {
...
function patternEquals(p: Pattern, p2: Pattern) {
  if (
    Math.abs(p2.y - p.y) <= p2.moduleSize &&
    Math.abs(p2.x - p.x) <= p2.moduleSize
  ) {
...
type Runs = number[]
/**
 * Since pattern means runs of identical color (dark or white), we cannot have
 * pattern like [true, true], because it will be hard to separate same color
 * runs.
 *
  ...
  const res = {
    ...
    check(
      ...
      const check = (p: number, step: Point) => {
        for (
          ;
          b.isInside(i) && !!b.point(i) === res.pattern[p];
          pointIncr(i, step)
        ) {
          runs[p]++
          j++
  ...
  for (let y = ySkip - 1; y < b.height && !done; y += ySkip) {
    FINDER.scanLine(b, y, 0, b.width, (runs, x) => {
      ...
      if (skipped) {
        ...
        skipped = true
        const d = int(
          (Math.abs(q[0].x - q[1].x) - Math.abs(q[0].y - q[1].y)) / 2,
        )
  ...
  for (let i = 0; i < flen - 2; i++) {
    ...
    for (let j = i + 1; j < flen - 1; j++) {
      ...
      for (let k = j + 1; k < flen; k++) {
        ...
        const arr = [square0, distance2(fj, fk), distance2(fi, fk)].sort(
          (a, b) => a - b,
        )
        const a = arr[0]
        const b = arr[1]
...
function readInfoBits(b: Bitmap) {
  const readBit = (x: number, y: number, out: number) =>
    (out << 1) | (b.data[y][x] ? 1 : 0)
  const size = b.height
  // Version information
  let version1 = 0
  for (let y = 5; y >= 0; y--)
...
function decodeBitmap(
  b: Bitmap,
  decoder: (bytes: Uint8Array) => string = bytesToUtf8,
): string {
  const size = b.height
  ...
  while (true) {
    ...
    if (mode === "numeric") {
      ...
      while (count >= 2) {
        ...
        res += info.alphabet.alphanumerc
          .encode([Math.floor(v / 45), v % 45])
          .join("")
        count -= 2
      }
      if (count === 1)
...
function cropToSquare(img: Image) {
  ...
  return {
    offset,
    img: { height: squareSize, width: squareSize, data: croppedData },
  }
}
...
export function decodeQR(img: Image, opts: DecodeOpts = {}): string {
  for (const field of ["height", "width"] as const) {
    if (!Number.isSafeInteger(img[field]) || img[field] <= 0)
      throw new Error(
        `invalid img.${field}=${img[field]} (${typeof img[field]})`,
      )
  }
  if (
